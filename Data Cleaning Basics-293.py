...
new_columns = []
new_columns = map(lambda x: x.strip(), laptops.columns)
...
def clean_name(name):
    cleaned = name.strip()
    cleaned = cleaned.replace('Operating System', 'os')
    cleaned = cleaned.replace(' ', '_')
    cleaned = cleaned.replace('(', '').replace(')', '')
    return cleaned.lower()

laptops.columns = map(lambda x: clean_name(x), laptops.columns)
...
laptops.columns = map(lambda x: x if x != 'weight' else 'weight_kg', laptops.columns)
